...
func food(food: String) -> Meal? {
    if let mealInDicionary = meals["\(food)"] { //인풋값이 딕셔너리 안에 있는지 확인
        if mealInDicionary.food != nil && mealInDicionary.calories != nil { //Meal 타입 내 저장속성들이 채워져있는지 확인
            return mealInDicionary.self
        } else {
            return nil
        }
    } else {
